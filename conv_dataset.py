...
class Dataset(torch.utils.data.Dataset):
    ...
    def _convnet_load_data(self, wav_path: str, random_scale) -> torch.Tensor:
        ...
        with torch.no_grad():
            out = convnet(scaled_tensor.to("cpu").unsqueeze(0).unsqueeze(0).detach())
...
class Collater(object):
    ...
    def __call__(self, batch: List) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        max_lenght_mel_tensor = self.max_t_mel   # 5 token of sos and 5 token of bos
        
        max_lenght_ref_mel_tensor = self.max_t_mel 
        ...
        for bid, (mel, ref_mel, max_len) in enumerate(batch):
            ...
            ref_mel_lengths[bid] = ref_mel.shape[0]
            
            # Attach to batch and padding
            padded_mel_tensor[bid, :mel.shape[0], :] = mel
            padded_ref_mel_input_tensor[bid, :ref_mel.shape[0], :] = ref_mel
            padded_ref_mel_target_tensor[bid, :ref_mel.shape[0], :] = ref_mel

            

        return mel_lengths, ref_mel_lengths, padded_mel_tensor.permute(0,2,1), padded_ref_mel_input_tensor, padded_ref_mel_target_tensor.permute(0,2,1).unsqueeze(1), mel_mask, ref_mel_input_mask, mel_padding_mask, ref_mel_input_padding_mask
